...
class S2RestClientTest {
	...
	public void shouldNormalizeBaseUrlOnInit() {
		...
		sut.s2url = urlWithoutEndingSlash
		// When 
		sut.init()
		// Then		
	...
	public void shouldDeleteFileByPath() {
		 // Given
		 mockServer.expect(requestTo("${BASE_URL}/${ANY_FILE_PATH}?path=true"))
			.andExpect(method(HttpMethod.DELETE))
			.andRespond(withSuccess());
		ContentStream contentStream = new SimpleContentStream(stream)
		// When
	...
	public void shouldDeleteFolder() {
		 // Given
		 mockServer.expect(requestTo("${BASE_URL}/folders/${ANY_FOLDER_PATH}"))
			.andExpect(method(HttpMethod.DELETE))
			.andRespond(withSuccess());
		ContentStream contentStream = new SimpleContentStream(stream)
		// When
		sut.deleteFolder(ANY_FOLDER_PATH)
		// Then
		mockServer.verify()
	}
    ...
    public void getFid() {
		// Given
		mockServer.expect(requestTo("${BASE_URL}/keys/${ANY_PATH}"))
		   .andExpect(method(HttpMethod.GET))
		   .andRespond(withSuccess(ANY_FID, MediaType.TEXT_PLAIN));
	   // When
	   String path = sut.getPath(ANY_PATH)
	   // Then
	   mockServer.verify()
...
//		// Given
//		def finfo1 = new SimpleFileInfo("flid1", "filename1", "extension1", 1)
//		def finfo2 = new SimpleFileInfo("flid2", "filename2", "extension2", 2)
//		def finfo3 = new SimpleFileInfo("flid3", "filename3", "extension3", 3)
//		
//		mockServer.expect(requestTo("${BASE_URL}/rpc/${ANY_OP_NAME}"))
//			.andExpect(method(HttpMethod.POST))
//			.andRespond(withSuccess(jsonResponse, MediaType.APPLICATION_JSON));
